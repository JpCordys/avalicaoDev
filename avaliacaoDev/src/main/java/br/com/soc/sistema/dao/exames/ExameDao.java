...
public class ExameDao extends Dao {
	...
	public void editarExame(ExameVo exameVo) {
		ExameVo exame = findByCodigo(Integer.parseInt(exameVo.getRowid()));
        String query = "UPDATE nomes SET (nm_exame) WHERE id=?";
	...
	public void deleteExame(ExameVo exameVo) {
		ExameVo exame = findByCodigo(Integer.parseInt(exameVo.getRowid()));
		String query = "DELETE rowid FROM exames WHERE id=?";
		
		try{
			Connection con = getConexao();
			PreparedStatement ps = con.prepareStatement(query);
			
			ps.setString(1, exame.getRowid());
			con.commit();
		}
